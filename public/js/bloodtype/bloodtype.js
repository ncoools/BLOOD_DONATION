...
$(document).on('click', '.edit-btn', function () {
   ...
   $.ajax({
    url: baseUrl + 'bloodtype/edit/' + userId,
    method: 'GET',
    dataType: 'json',
    success: function (response) {
        if (response.data) {
...
$(document).on('click', '.deleteUserBtn', function () {
    ...
    if (confirm('Are you sure you want to delete this bloodtype?')) {
        $.ajax({
            url: baseUrl + 'bloodtype/delete/' + userId,
            method: 'POST', 
            data: {
                _method: 'DELETE',
                [csrfName]: csrfToken
...
$(document).ready(function () {
    ...
    $table.DataTable({
        ...
        serverSide: true,
        ajax: {
            url: baseUrl + 'bloodtype/fetchRecords',
            type: 'POST',
            headers: {
        ...
        columns: [
        { data: 'row_number' },
        { data: 'bloodtype_id', visible: false },
        { data: 'bloodtype' },
      
        {
            ...
            render: function (data, type, row) {
                return `
                <button class="btn btn-sm btn-warning edit-btn" data-id="${row.bloodtype_id}">
                <i class="far fa-edit"></i>
                </button>
                <button class="btn btn-sm btn-danger deleteUserBtn" data-id="${row.bloodtype_id}">
